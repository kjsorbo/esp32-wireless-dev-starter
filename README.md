...
bool otaPerformUpdate() {
    ...
    // Disable watchdog during long download (can take 30+ seconds)
...
bool checkDoubleReset() {
    esp_task_wdt_reset();  // Feed watchdog during this check
    unsigned long now = millis();

    // If reset within 3 seconds AND resetCount is valid (not garbage)
    if (resetCount > 0 && resetCount < 10 && (now < 3000)) {
        resetCount = 0;
        return true;
    }

    // Set to 1 (not increment - avoids garbage accumulation)
    resetCount = 1;

    // Wait 3 seconds, feeding watchdog every 100ms
    for (int i = 0; i < 30; i++) {
        esp_task_wdt_reset();
        delay(100);
    }
...
void setup() {
    ...
    // Add current task to watchdog (Arduino framework already initialized WDT)
    esp_task_wdt_add(NULL);
...
# ============================================================================
# Ensure SHARED syslog listener is running (multi-project)
# ============================================================================

echo ""
echo -e "${YELLOW}Checking syslog infrastructure...${NC}"

# Path to shared syslog listener (in projects root, not per-project)
SHARED_SYSLOG_DIR="/home/$SERVER_USER/projects"
SHARED_SYSLOG_SCRIPT="$SHARED_SYSLOG_DIR/shared_syslog_listener.py"
SHARED_SYSLOG_LOG="$SHARED_SYSLOG_DIR/syslog_listener.log"

# Step 1: Kill any OLD per-project syslog listeners (they break multi-project setup)
echo -e "${BLUE}Checking for old per-project listeners...${NC}"
# NOTE: Use ps aux instead of pgrep - pgrep can return stale PIDs on some systems
...
# Step 2: Check if shared listener is running
SHARED_RUNNING=$(ssh $SERVER_USER@$SERVER_HOST "ps aux | grep 'shared_syslog_listener' | grep -v grep | awk '{print \$2}' | head -1" 2>/dev/null || echo "")
...
#!/usr/bin/env python3
\"\"\"
Shared multi-project syslog listener.
Routes messages to project-specific log files based on project name in syslog message.
ESP32 sends: <priority>project-name: message
\"\"\"
import socket
import os
import re
...
def main():
    ...
    print(f\"Shared syslog listener running on UDP port 514...\", flush=True)
    print(f\"Routing logs to: {BASE_DIR}/<project>/logs/esp32.log\", flush=True)
    ...
    while True:
        ...
        # Parse project name from syslog message
        ...
        # Create project log directory if needed
...
EOFPY"
    ...
    ssh $SERVER_USER@$SERVER_HOST "nohup sudo python3 $SHARED_SYSLOG_SCRIPT > $SHARED_SYSLOG_LOG 2>&1 &"
    sleep 1

    # Verify it started
    SHARED_RUNNING=$(ssh $SERVER_USER@$SERVER_HOST "ps aux | grep 'shared_syslog_listener' | grep -v grep | awk '{print \$2}' | head -1" 2>/dev/null || echo "")
    if [ -n "$SHARED_RUNNING" ]; then
        echo -e "${GREEN}Shared syslog listener started (PID: $SHARED_RUNNING)${NC}"
    else
    ...
    echo -e "${GREEN}Shared syslog listener already running (PID: $SHARED_RUNNING)${NC}"
fi
...
| `{NVS_NAMESPACE}` | First 13 chars of PROJECT_NAME (max 13 to be safe) |
